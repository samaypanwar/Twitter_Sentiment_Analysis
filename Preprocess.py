...
class Preprocessing:
    ...
    # to take in a dataframe in the initialization
    # function class_imbalance checks whether our classes are properly distributed or not 
    # the threshold for the class imbalance can be explicitly set   
    def class_imbalance(self,dataset,threshold,target='Polarity',num_classes=2):
        ...
        for i in range(num_classes):
            ...
            if (1-threshold)*l<c[i]<(1+threshold)*l:
                ...
                # if true then there is a class imbalance in one of our classes
                return True
           # if false then our classes are within the threshold     
        return False

    
    #to clean tweets we use the class PreProcessTweets
    class PreProcessTweets:
        ...
        def processTweets(self,list_of_tweets):
            ...
            #returns an entire list of tweets
            return processedTweets
    
        #cleans the text, i.e removes the AT_User and any urls present in the tweet
    ...
    #  a function to remove the common abbreviations in the data
    ...
    # we use pos_tagging here for the major tags such as nouns, verbs, adjectives and adverbs
    #rest of the tags are classified as nouns if they not in the above category
    ...
    # function to lemmatize the text, i.e return the word to its root
    def lemmatize(self,text):
        ...
        for index,word in enumerate(text):
            if word not in stopword:
                ...
                string.append(word)
        string=" ".join(string)
        # returns a lemmatized string of text
        return string
    
    def __call__(self):
        #this function is called by default when the class is initialized
        ...
        #the texts are converted to sequences here using a keras function
        word_index=tokenizer.word_index
        data=pad_sequences(sequences,maxlen=MAX_SEQUENCE_LENGTH)
        # we pad all sentences to MAX_SEQUENCE_LENGTH with 0 in the beginning

        #one probelm is that we need word_index for our embeddings but have not returned that yet
